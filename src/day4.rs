...
fn parse_ranges(input: &str) -> Vec<(Range<u32>, Range<u32>)> {
    input
        ...
        .map(|l| {
            ...
            let a_start = u32::from_str_radix(a_start, 10).expect("unable to parse a_start");
            let a_end = u32::from_str_radix(a_end, 10).expect("unable to parse a_end");

            let (b_start, b_end) = b.split_once('-').expect("could not find b_start and b_end");
            let b_start = u32::from_str_radix(b_start, 10).expect("unable to parse b_start");
            let b_end = u32::from_str_radix(b_end, 10).expect("unable to parse b_end");
